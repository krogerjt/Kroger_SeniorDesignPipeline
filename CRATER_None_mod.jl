...
function findCliques(mat, mat_cor, delta;
	...
	#Remove CFG in BK L1
	BronKerbosch(mat, R, P, X, output, min_size=min_size)

    return nothing
end

function CRATER_None(mat, delta, sigma; pair_lim=1)
    sz = size(mat,1)
    ...
    for idx=idx_start:length(pairs)
		...
		cliques_new = assessMultipoles(mat, cliques_new, delta=delta, sigma=sigma)
        cliques_new == [] && (continue)
